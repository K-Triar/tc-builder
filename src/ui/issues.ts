...
export const ISSUE_HINTS: Record<IssueCode, string> = {
  ...
  ROUTE_CODE_CONFLICT:
    '同じ区間でも通るのりばが違う列車の走り方があります。通る駅ののりばを見直すか、「経路に入れる駅」の上書きで揃えてください。',
  FORMATION_CONFLICT:
    '同じ形式コードの列車に違う設定（最高速度など）が付いています。列車の走り方ごとの形式コードか設定を揃えてください。',
  ...
  KIND_NO_STOP:
    'この種別の列車は、どこからも出発しない設定になっています。停車駅の表で ○ を付けるか、この走り方から外してください。',
  SKIP_MANUAL:
    '止まる列車と通過する列車の組み合わせが複雑で、看板の条件を自動で決められません。試運転で確かめながら手で調整してください。',
  FOREIGN_FORMATION_MISSING:
    '直通先の鉄道会社が決めた編成名を聞いて入れてください。空のままだとこの列車のコマンドは出ません。',
  FOREIGN_STATION:
    '相手の鉄道会社の駅です。行先コードとのりば番号が相手の設定と合っているか確かめてください（直さなくても看板とコマンドは出ます）。',
  FORM_PARITY:
    'KT式では下りは偶数、上りは奇数の形式番号を使います。列車の走り方の向きか形式コードを見直してください。',
  TERMINAL_THROUGH:
  ...
  PLATFORM_MISSING:
    '列車の走り方が通るのりばが駅にありません。駅にのりばを足すか、走り方ののりばを選び直してください。',
};
