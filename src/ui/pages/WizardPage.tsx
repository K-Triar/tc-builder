...
const INTRO: readonly string[] = [
  'この路線網の名前と、どの鉄道会社かを確かめます。鉄道会社コードと種別は入力済みです。路線のコードがまだなければ、ここで入れます。',
  '列車が通る駅を、路線の端から順に登録します。止まらずに通過するだけの駅や、直通先の他の鉄道会社の駅も入れます。駅コードは自動で付きます。',
  'ホームに立って線路を見たとき、列車が左右どちらへ出ていくかを、駅ごとに選びます。これで看板を並べる順番が決まります。',
  '列車がどこからどこへ、どの駅を通って走るかを決めます。種別（普通・快速など）ごとに、止まる駅に ○ を付けます。',
