...
1. **はじめる**：ホームで「新しいプロジェクト」（Kトライアのプリセットか空の団体）を作るか、「サンプル（瑠璃線系統）を開く」で完成例を見ます。
2. **ウィザード**で順に答えます：団体 → 路線 → 種別 → 駅 → のりば（ホームから見て列車が進む向き）→ 系統（通る駅とのりば、種別ごとの停車 ○/×）→ 確認 → 完成。後からは「編集」画面で表形式でまとめて直せます。
